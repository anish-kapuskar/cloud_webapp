...
public class UserDAOImpl implements UserDAO {
  ...
  public void register(User user) {
   ...
   if(users.size() == 0){
    ...
    jdbcTemplate.update(sql2, new Object[] { user.getEmail(),BCrypt.hashpw(user.getPassword(), BCrypt.gensalt()) , user.getFirstname(),
    user.getLastname() });
    }
...
jdbcTemplate.update(updateQuery, user.getPassword(), user.getFirstname(),user.getLastname(),user.getEmail());
    ...
    public User validateUser(Login login) {
        ...
        String hashpwd = BCrypt.hashpw(login.getPassword(),BCrypt.gensalt());
        String plainpwd = login.getPassword();
        
        if(BCrypt.checkpw(plainpwd, hashpwd)){

        
     String sql = "select * from users where email='" + login.getEmail()
 //    + "' and password='" + BCrypt.hashpw(login.getPassword(),BCrypt.gensalt())
    + "'";
                List<User> users = jdbcTemplate.query(sql, new UserMapper());
